...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If the node is at the head, then reuse RemoveHead which
                // already handles the single-item list case.
                if (curr == _head)
                {
                    RemoveHead();
                }
                // If the node is at the tail, then reuse RemoveTail.
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                // For any node in the middle, reconnect the links to
                ...
                return; // We can exit the function after removing the first match
            }
    ...
    {
        // Search the whole list, replacing every node that matches
        // 'oldValue'.  Unlike Remove, we do not stop at the first match.
        Node? curr = _head;
        while (curr is not null)
        {
            ...
            curr = curr.Next; // Go to the next node to keep searching
        }
    }
